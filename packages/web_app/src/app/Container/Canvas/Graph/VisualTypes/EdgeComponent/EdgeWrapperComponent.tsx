    ...
    <Group>
      <Group>
        ...
        <Line
          ...
          stroke="transparent"
          strokeWidth={10}
          onMouseDown={(e) => mouseDownHandler(e, { mouseOn: 'EDGE' })}
        />
      </Group>
      {props.points.slice(1, props.points.length - 2).map((p, i) => (
        <Group key={`lineseg-${i}`}>
          <Line
            points={[p.x, p.y, props.points[i + 2].x, props.points[i + 2].y]}
            stroke={color}
            strokeWidth={strokeWidth}
          />
          <Line
            ...
            onMouseDown={(e) =>
              mouseDownHandler(e, {
                ...
                moveInfo: {
                  edgePieceInd: i,
                  inputPortLoc:
                    i % 2 === 0 ? props.points[props.points.length - 1].x : props.points[props.points.length - 1].y,
      ...
      <Group>
        <Line
          points={props.points
            .slice(props.points.length - 2, props.points.length)
            .map((p) => [p.x, p.y])
            .flat()}
          stroke={color}
          strokeWidth={strokeWidth}
