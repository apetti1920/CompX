...
// @ts-ignore
// eslint-disable-next-line import/no-unresolved
import { LinearInterp } from 'compx_common/Helpers/Other';
// eslint-disable-next-line import/no-unresolved,@typescript-eslint/ban-ts-comment
// @ts-ignore
// eslint-disable-next-line import/no-unresolved
...
function GraphReducer(state: StateType, action: ActionPayloadType): StateType {
  switch (action.type) {
    case MovedBlockActionType: {
      ...
      tempState.currentGraph.graph.blocks
        .filter((block) =>
        ...
        .forEach((block) => {
          ...
          block.position = new Vector2D(block.position.x + delta.x, block.position.y + delta.y);
        });
    ...
    case ResizedBlockActionType: {
      ...
      tempState.currentGraph.graph.blocks[blockInd].position = Vector2D.add(
        tempState.currentGraph.graph.blocks[blockInd].position,
        Vector2D.multiplyVec(delta, Vector2D.multiply(sizeMultiplier, 0.5))
      );
      tempState.currentGraph.graph.blocks[blockInd].size = Vector2D.add(
    ...
    case SelectedObjectActionType: {
      ...
      if (objectType === 'BLOCK')
        tempState.currentGraph.selected.push({
          itemType: 'BLOCK',
          id: objectId
        });
    ...
    case DeletedObjectActionType: {
      ...
      tempState.currentGraph.graph.edges = tempState.currentGraph.graph.edges.filter(
        (e) =>
          !selectedEdges.includes(e.id) &&
          !selectedBlocks.includes(e.output.blockID) &&
          !selectedBlocks.includes(e.input.blockID)
      );
      tempState.currentGraph.graph.blocks = tempState.currentGraph.graph.blocks.filter(
        (b) => !selectedBlocks.includes(b.id)
      );
      tempState.currentGraph.selected = [];
    ...
    case AddEdgeActionType: {
      ...
      if (
        tempState.currentGraph.graph.edges.find(
          (e) =>
            (e.output.blockID === outputBlock.id &&
              e.output.portID === outputPort.id &&
              e.input.blockID === inputBlock.id &&
              e.input.portID === inputPort.id) ||
      ...
      const outputPortLoc = CalculatePortLocation(
        ...
        tempState.userStorage.canvas.zoom,
      ...
      const tmpMidPercent =
        0.5 *
        ((outputPortInd + 1) / outputBlock.outputPorts.length + (inputPortInd + 1) / inputBlock.inputPorts.length);
      const midPercentTmp = LinearInterp(tmpMidPercent, 0, 1, dir1 ? 0.75 : 0.25, dir1 ? 0.25 : 0.75);

      const newRadius = 20 * (1 + midPercentTmp) ** 2;
    ...
    case AddEdgeSplitActionType: {
      ...
      if (
        afterEdgePieceInd < 0 ||
        afterEdgePieceInd >= tempState.currentGraph.graph.edges[selectedEdgeInd].midPoints.length
      )
        return tempState;
      ...
      if (tempPoints.length === 1) {
        console.log('case 1');
        next1 = 0.5;
        next2 = (tempPoints[0] + 1.0) / 2.0;
      } else if (afterEdgePieceInd + 2 < tempPoints.length) {
        console.log('case 2');
        next1 =
          ((afterEdgePieceInd - 1 >= 0 ? tempPoints[afterEdgePieceInd - 1] : 0.0) + tempPoints[afterEdgePieceInd + 1]) /
          2.0;
        next2 = (tempPoints[afterEdgePieceInd] + tempPoints[afterEdgePieceInd + 2]) / 2.0;
      } else if (afterEdgePieceInd - 2 >= 0) {
        console.log('case 3');
        next1 =
          ((afterEdgePieceInd + 1 < tempPoints.length ? tempPoints[afterEdgePieceInd + 1] : 1.0) +
            tempPoints[afterEdgePieceInd - 1]) /
          2.0;
        next2 = (tempPoints[afterEdgePieceInd] + tempPoints[afterEdgePieceInd - 2]) / 2.0;
      } else {
        console.log('case 4');
