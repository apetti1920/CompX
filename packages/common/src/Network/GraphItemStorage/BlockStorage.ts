...
interface TempIOsWithIDType<Inputs extends PortStringListType, Outputs extends PortStringListType> {
  inputPorts: MapStringsToPortStoragesWithIDType<Inputs>;
  outputPorts: MapStringsToPortStoragesWithIDType<Outputs>;
}
export interface BlockStorageWithIDType<Inputs extends PortStringListType, Outputs extends PortStringListType>
  extends WithID,
    Omit<BlockStorageType<Inputs, Outputs>, 'inputPorts' | 'outputPorts'>,
    TempIOsWithIDType<Inputs, Outputs> {}

export function isBlockStorageWithIDType<Inputs extends PortStringListType, Outputs extends PortStringListType>(
  obj: any
): obj is BlockStorageWithIDType<Inputs, Outputs> {
  if (!isBlockStorageType(obj)) return false;
  if (!('id' in obj) || typeof obj['id'] !== 'string') return false;

  if (
    !Array.isArray(obj['outputPorts']) ||
    !(obj['outputPorts'] as any[]).every((p: any) => isPortStorageWithIDType(p))
  )
    return false;
  if (!Array.isArray(obj['inputPorts']) || !(obj['inputPorts'] as any[]).every((p: any) => isPortStorageWithIDType(p)))
    return false;
