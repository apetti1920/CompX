...
type StorageSelect<T> = T extends Port<infer V>
  ? PortStorageType<V>
  : T extends Block<infer V, infer U>
  ? BlockStorageType<V, U>
  : T extends Edge<infer V>
  ? EdgeStorageType<V>
  : T extends Graph
  ? GraphStorageType
  : never;
type StorageWithIDSelect<T> = T extends Port<infer V>
  ? PortStorageWithIDType<V>
  : T extends Block<infer V, infer U>
  ? BlockStorageWithIDType<V, U>
  : T extends Edge<infer V>
  ? EdgeStorageType<V>
  : T extends Graph
  ? StorageSelect<T>
...
abstract class GraphObjectBase<T> {
...
export default GraphObjectBase;
