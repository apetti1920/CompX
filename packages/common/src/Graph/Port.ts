...
export class Port<U extends keyof PortTypes> implements PortStorageType<U>, GraphObject<Port<U>> {
  ...
  private constructor(portName: string, parentId: string, type: U, initialValue?: PortTypes[U]) {
    this.id = uuidV4();
    this.name = portName;
    this.type = type;
  ...
  public static InitializeFromStorage<U extends keyof PortTypes>(
    portStorage: PortStorageType<U>,
    parentId: string
  ): Port<U> {
    return new Port(portStorage['name'], parentId, portStorage['type'], portStorage['initialValue']);
  }

  // Gets the current Value of the port
  public GetObjectValue(): PortTypes[U] | never {
    if (this._objectValue !== undefined) return this._objectValue;
